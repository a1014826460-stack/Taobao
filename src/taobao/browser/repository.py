...
''')
    ...
    def claim_next_task(self,account_id,now):
        with self.conn:
            ...
            self.conn.execute('UPDATE accounts SET last_used_at=?,updated_at=? WHERE account_id=?',(t,t,account_id)); return dict(self.conn.execute('SELECT * FROM crawl_tasks WHERE task_id=?',(row['task_id'],)).fetchone())
