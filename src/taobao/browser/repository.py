...
''')
    ...
    def upsert_account(self,account:AccountRecord):
        ...
        self.conn.execute('''INSERT INTO accounts(account_id,cookie_source,status,pause_reason,created_at,updated_at) VALUES(?,?,?,?,?,?) ON CONFLICT(account_id) DO UPDATE SET cookie_source=excluded.cookie_source,status=excluded.status,pause_reason=excluded.pause_reason,updated_at=excluded.updated_at''',(account.account_id,account.cookie_source,status,account.pause_reason,t,t))
    def pause_account(self,account_id,reason): self.conn.execute('UPDATE accounts SET status="paused",pause_reason=?,updated_at=? WHERE account_id=?',(reason,_now(),account_id))
    ...
    def enqueue_keyword(self,keyword,platform,page_limit=3,run_id=None):
        ...
        for p in range(1,page_limit+1):
            ...
            self.conn.execute('''INSERT INTO crawl_tasks(task_id,task_type,keyword,platform,page_no,status,next_run_at,run_id,created_at,updated_at) VALUES(?,?,?,?,?,"pending",?,?,?,?) ON CONFLICT(task_type,keyword,page_no) DO UPDATE SET platform=excluded.platform,run_id=COALESCE(excluded.run_id,crawl_tasks.run_id),updated_at=excluded.updated_at''',(tid,'keyword',keyword,platform,p,t,run_id,t,t))
            row=self.conn.execute("SELECT task_id FROM crawl_tasks WHERE task_type='keyword' AND keyword=? AND page_no=?",(keyword,p)).fetchone(); ids.append(row['task_id'])
        return ids
    ...
    def claim_next_task(self,account_id,now):
        with self.conn:
            ...
            t=_now(); self.conn.execute('UPDATE crawl_tasks SET status="running",account_id=?,attempts=attempts+1,updated_at=? WHERE task_id=?',(account_id,t,row['task_id'])); self.conn.execute('UPDATE accounts SET last_used_at=?,updated_at=? WHERE account_id=?',(t,t,account_id)); return dict(self.conn.execute('SELECT * FROM crawl_tasks WHERE task_id=?',(row['task_id'],)).fetchone())
    ...
    def save_network_record(self,record):
        r=dict(record); rid=r.get('record_id') or str(uuid.uuid4()); self.conn.execute('''INSERT OR REPLACE INTO network_records(record_id,run_id,account_id,page_type,url,method,status_code,resource_type,response_headers_json,response_body,body_sha256,captured_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)''',(rid,r.get('run_id'),r.get('account_id'),r.get('page_type'),r.get('url'),r.get('method'),r.get('status_code'),r.get('resource_type'),_json(r.get('response_headers',r.get('response_headers_json',{}))),r.get('response_body'),r.get('body_sha256'),r.get('captured_at') or _now()))
