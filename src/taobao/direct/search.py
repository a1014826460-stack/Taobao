"""Concurrent, resumable crawler for Fan-B's Taobao ``item_search`` API.
...
SEARCH_API_URL = "https://api-gw.fan-b.com/taobao/item_search/"
...
class SearchCrawlerConfig:
    ...
    secret: str
    query: str
    db_path: str = "data/taobao_search.sqlite3"
    max_pages: int = 1
    max_workers: int = 4
    sort: str = "_sale"
    start_price: str = "0"
...
def query_fingerprint(config: SearchCrawlerConfig) -> str:
...
def build_search_request(config: SearchCrawlerConfig, page: int) -> Request:
    ...
    return Request(
        ...
        headers={"User-Agent": "taobao-item-search-crawler/1.0"},
    )
...
class SQLiteSearchStore:
    ...
    def get_page_state(self, fingerprint: str, page: int) -> dict[str, Any] | None:
        row = self.conn.execute(
            "SELECT * FROM search_state WHERE query_fingerprint = ? AND page = ?",
    ...
    def count_pages(self, fingerprint: str) -> int:
        row = self.conn.execute(
            "SELECT COUNT(*) AS total FROM search_pages WHERE query_fingerprint = ?", (fingerprint,)
        ).fetchone()
        return int(row["total"])

    def count_items(self, fingerprint: str) -> int:
        row = self.conn.execute(
            "SELECT COUNT(*) AS total FROM search_items WHERE query_fingerprint = ?", (fingerprint,)
        ).fetchone()
        return int(row["total"])

    def get_item(self, fingerprint: str, num_iid: str) -> dict[str, Any] | None:
        row = self.conn.execute(
            "SELECT * FROM search_items WHERE query_fingerprint = ? AND num_iid = ?",
    ...
    def mark_error(self, fingerprint: str, page: int, error: Exception) -> None:
        ...
        with self.conn:
            self.conn.execute(
                """
                INSERT INTO search_state (query_fingerprint, page, status, last_error, updated_at)
                VALUES (?, ?, 'error', ?, ?)
...
def fetch_search_page(config: SearchCrawlerConfig, page: int, opener=urlopen) -> dict[str, Any]:
    """Request one page with retries; all gateway/API errors remain visible to the caller."""
...
def crawl_search(
    ...
    fetcher: Callable[[SearchCrawlerConfig, int], dict[str, Any]] | None = None,
) -> SearchCrawlResult:
    ...
    store = SQLiteSearchStore(config.db_path)
    try:
        ...
        if fetcher is None:
            ...
            def default_fetcher(crawler_config: SearchCrawlerConfig, page: int) -> dict[str, Any]:
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Concurrent, resumable Taobao item_search crawler")
    ...
    parser.add_argument("--db", default="data/taobao_search.sqlite3", help="SQLite database path.")
    parser.add_argument("--sort", default="_sale", help="Search sort; _sale orders by sales (default: _sale).")
...
def config_from_args(args: argparse.Namespace) -> SearchCrawlerConfig:
    _load_dotenv()
