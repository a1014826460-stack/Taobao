...
DEFAULT_DB = Path("data") / "taobao_shop_items.sqlite3"
DEFAULT_OUTPUT = Path("data") / "item_raw_json.xlsx"
...
def export_item_details_to_xlsx(db_path, output_path, num_iids=None):
    db_rows = load_item_detail_rows(db_path, num_iids)
    rows = []
    raw_headers = []
    ...
    worksheet.title = "商品信息"
    worksheet.append(headers)

    written = 0
    for db_row, response, flat_raw in rows:
        for export_row in build_export_rows(db_row, response=response):
...
def load_item_detail_rows(db_path, num_iids=None):
    ...
    try:
        if num_iids:
            ...
            sql = f"""
                ...
                WHERE num_iid IN ({placeholders})
                ORDER BY CASE num_iid
            """
            order_params = []
        ...
        return [
            dict(row)
            for row in conn.execute("SELECT * FROM item_details ORDER BY updated_at, num_iid").fetchall()
        ]
    finally:
        conn.close()


def build_export_rows(db_row, response=None):
    if response is None:
        response = json.loads(db_row["raw_json"])
    ...
    rows = []
    for sku in skus:
        rows.append(build_export_row(db_row, response, item, sku))
    return rows
...
def build_export_row(db_row, response, item, sku):
    ...
    return {
        "平台名称": platform_name(response),
        "商品ID": text(item.get("num_iid") or db_row.get("num_iid")),
        "spu名称": text(item.get("title") or db_row.get("title")),
        "skuid": text(sku.get("sku_id")),
        "sku描述": text(sku.get("properties_name") or sku.get("properties")),
        "正常价格（标价）": text(
            first_present(
        ...
        "店铺文本ID": text(item.get("shop_id") or db_row.get("shop_id") or item.get("seller_id")),
        "店铺链接": text(seller_info.get("zhuy") or shop_url(item.get("shop_id") or db_row.get("shop_id"))),
...
def platform_name(response):
    api_type = text(response.get("api_type")).lower()
    if api_type == "taobao":
        return "淘宝"
    return text(response.get("api_type")) or "淘宝"


def sales_status(item, sku):
    quantity = first_present(sku.get("quantity"), item.get("num"))
    if quantity == "":
        return ""
    try:
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="Export item_details.raw_json rows from SQLite to an XLSX file."
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Specific num_iid values. Supports comma, whitespace, and repeated arguments.",
    )
    parser.add_argument(
        "--num-iids-file",
        action="append",
        default=[],
        help="Text file containing num_iid values separated by comma, whitespace, or newline.",
    )
    return parser
...
def main(argv=None):
    ...
    written = export_item_details_to_xlsx(args.db, args.output, num_iids or None)
