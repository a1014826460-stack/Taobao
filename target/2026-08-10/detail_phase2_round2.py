...
CANDIDATE_CSV = OUTDIR / 'phase2_round2_detail_candidates.csv'
LOG = OUTDIR / 'detail_run_phase2_round2.log'
...
for kw in KEYWORDS:
    ...
    ''', (kw,)):
        ...
        if not iid or iid in seen:
            continue
        seen.add(iid)
        rows.append({'keyword': kw, 'sort': r['sort'] or '', 'page': int(r['page']), 'num_iid': iid, 'title': r['title'] or ''})
        picked += 1
...
log(f'JD_DETAIL_PHASE2_ROUND2 start source_rows={len(rows)} unique_iids={len(unique_rows)} marker={MARKER} workers={WORKERS}')
idx = 0
...
try:
    ...
    store.close()
log(f'JD_DETAIL_PHASE2_ROUND2 finished processed={processed} fetched={fetched} failed={failed} skipped={skipped} stopped={stop.is_set()}')
if stop.is_set():
    raise SystemExit(2)
